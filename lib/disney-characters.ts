...
const minifyResponse = (response: DisneyCharacter[]) => {
  const characters = response.map((character: DisneyCharacter) => {
    return {
      _id: character._id,
      name: character.name,
...
export const getAllDisneyCharacters = async () => {
  try {
    const DISNEY_API_ROUTE = 'https://api.disneyapi.dev/character'; // only returns first 50 characters for now
